...
from my_parser import main as parse_main
from my_parser import ProblemInstance
        ...
        if constraint_n in problem.variables[var2_name].constraints:
            ...
            if reference_name in problem.relations:
                ...
                if relation_type == 'supports':
                    # print(f'Testing if {val_tuple} in {relation.pairs}')
                    return val_tuple in relation.pairs
                elif relation_type == 'conflicts':
                    # print(f'Testing if {val_tuple} not in {relation.pairs}')
                ...
                # print(variable_order, relation_type, relation.pairs)
                ...
                # print(reference_name, cf.name, predicate_param_list, constraint_param_list, vvd)
                arg_dict = dict()
                param_count = len(predicate_param_list)
                for i in range(param_count):
                    if constraint_param_list[i] in vvd:
                        # print(predicate_param_list[i], constraint_param_list[i], vvd[constraint_param_list[i]])
                        arg_dict[predicate_param_list[i]] = vvd[constraint_param_list[i]]
                    else:
                        arg_dict[predicate_param_list[i]] = constraint_param_list[i]
                        # print(predicate_param_list[i], constraint_param_list[i])
                # print('arg_dict', arg_dict)
                evaluation = cf.evaluate(arg_dict)
                # print(vvd, arg_dict, evaluation)
                # print('====')
                return evaluation
    return True
...
def enforce_node_consistency():
    ...
    for unary_constraint in unary_constraints:
        ...
        if reference_name in problem.relations:
            ...
            relation_type = relation.semantics
            if relation_type == 'supports':
                # print(f'Restricting domain of {var} from {current_domains[var]} to  {relation.pairs}')
                current_domains[var] = relation.pairs
            else:
                # print('Node consistency not enforced with supports')
                pass
        elif reference_name in problem.predicates:
            # print(f'Enforce enforce_node_consistency with of {reference_name} with instantiation')
            predicate = problem.predicates[reference_name]
            cf = predicate.custom_function
            predicate_param_list = predicate.parameter_list
...
def output_results():
    ...
    if args.output_format:
        ...
        # for var in current_domains:
        #    print(var, current_domains[var])
    else:
